...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  val perfEvents = new EventSets(Seq(
    new EventSet((mask, hits) => Mux(wb_xcpt, mask(0), wb_valid && pipelineIDToWB((mask & hits).orR)), Seq(
      ...
      ("arith", () => id_ctrl(decoder.wxd) && !(id_ctrl(decoder.isJal) || id_ctrl(decoder.isJalr) || id_ctrl(decoder.mem) || Option.when(usingFPU)(id_ctrl(decoder.fp)).getOrElse(false.B) || Option.when(usingMulDiv)(id_ctrl(decoder.mul)).getOrElse(false.B) || Option.when(usingMulDiv)(id_ctrl(decoder.div)).getOrElse(false.B) || id_ctrl(decoder.csr) =/= CSR.N)),
    ...
    new EventSet((mask, hits) => (mask & hits).orR, Seq(
      ...
      ("replay", () => replay_wb))
      ++ (if (!usingMulDiv) Seq() else Seq(
        ("mul/div interlock", () => id_ex_hazard && (ex_ctrl(decoder.mul) || ex_ctrl(decoder.div)) || id_mem_hazard && (mem_ctrl(decoder.mul) || mem_ctrl(decoder.div)) || id_wb_hazard && wb_ctrl(decoder.div))))
      ++ (if (!usingFPU) Seq() else Seq(
  ...
  val decoderParameter = org.chipsalliance.rocketcore.decoder.InstructionDecoderParameter(
    ...
    (org.chipsalliance.rvdecoderdb.fromFile.instructions(os.pwd / "dependencies" / "riscv-opcodes") ++
      ...
      .map(i => i.pseudoFrom.getOrElse(i))
      .toSeq
      .distinct,
    pipelinedMul,
  ...
  val id_illegal_insn = !id_ctrl(decoder.isLegal) ||
    Option.when(usingMulDiv)(id_ctrl(decoder.mul) || id_ctrl(decoder.div) && !csr.io.status.isa('m'-'a')).getOrElse(false.B) ||
  ...
  val mul = pipelinedMul.option {
    ...
    m.io.req.valid := ex_reg_valid && Option.when(usingMulDiv)(ex_ctrl(decoder.mul)).getOrElse(false.B)
  ...
  val rf_wdata = Mux(dmem_resp_valid && dmem_resp_xpu, io.dmem.resp.bits.data(xLen-1, 0),
                 ...
                 Mux(Option.when(usingMulDiv)(wb_ctrl(decoder.mul)).getOrElse(false.B), mul.map(_.io.resp.bits.data).getOrElse(wb_reg_wdata),
                 wb_reg_wdata))))
  ...
  val ex_cannot_bypass = ex_ctrl(decoder.csr) =/= CSR.N || ex_ctrl(decoder.isJalr) || ex_ctrl(decoder.mem) || Option.when(usingMulDiv)(ex_ctrl(decoder.mul)).getOrElse(false.B) || Option.when(usingMulDiv)(ex_ctrl(decoder.div)).getOrElse(false.B) || Option.when(usingFPU)(ex_ctrl(decoder.fp)).getOrElse(false.B) || Option.when(usingRoCC)(ex_ctrl(decoder.rocc)).getOrElse(false.B)
  ...
  val mem_cannot_bypass = mem_ctrl(decoder.csr) =/= CSR.N || mem_ctrl(decoder.mem) && mem_mem_cmd_bh || Option.when(usingMulDiv)(mem_ctrl(decoder.mul)).getOrElse(false.B) || Option.when(usingMulDiv)(mem_ctrl(decoder.div)).getOrElse(false.B) || Option.when(usingFPU)(mem_ctrl(decoder.fp)).getOrElse(false.B) || Option.when(usingRoCC)(mem_ctrl(decoder.rocc)).getOrElse(false.B)
