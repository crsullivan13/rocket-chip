...
class InstructionDecoder(p: InstructionDecoderParameter) {
  ...
  private val instructionDecodeFields: Seq[DecodeField[RocketDecodePattern, _ <: Data]] = Seq(
    ...
    (if (useMulDiv) Seq(mul, div) else None) ++
  ...
  object fp extends BoolDecodeField[RocketDecodePattern] {
    ...
    override def genTable(op: RocketDecodePattern): BitPat = if (Utils.isFP(op.instruction)) y else n
  }
  ...
  object memCommand extends UOPDecodeField[RocketDecodePattern] {
    ...
    override def genTable(op: RocketDecodePattern): BitPat = {
      op.instruction.name match {
        // format: off
        case i if Seq("amoadd.d", "amoadd.w", "amoand.d", "amoand.w").contains(i) => UOPMEM.xaAdd
  ...
  object aluFn extends UOPDecodeField[RocketDecodePattern] {
    ...
    override def genTable(op: RocketDecodePattern): BitPat = op.instruction.name match {
      ...
      case i if Seq("mulh", "mulhsu", "mulhu").contains(i) => UOPALU.mulh
  ...
  object selAlu2 extends UOPDecodeField[RocketDecodePattern] {
    ...
    override def genTable(op: RocketDecodePattern): BitPat = op.instruction.name match {
      // format: off
      case i if Seq("addi", "addiw", "aes64ks1i", "andi", "auipc", "bclri", "bclri", "bexti", "bexti", "binvi", "binvi", "bseti", "bseti", "csrrci", "csrrsi", "csrrwi", "fld", "flh", "flw", "fsd", "fsh", "fsw", "jalr", "lb", "lbu", "ld", "lh", "lhu", "lui", "lw", "lwu", "ori", "rori", "rori", "roriw", "sb", "sd", "sh", "slli", "slli", "slli.uw", "slliw", "slti", "sltiu", "srai", "srai", "sraiw", "srli", "srli", "srliw", "sw", "xori").contains(i) => UOPA2.imm
